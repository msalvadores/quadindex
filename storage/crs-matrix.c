...
static gdouble elapsed_res = 0;
static int elapsed_res_count = 0;

static gdouble elapsed_ins = 0;
static int elapsed_ins_count = 0;
...
crs_matrix *crs_matrix_new(GPtrArray *temp_values, crs_matrix_index *index_conf,guint id) {
...
size_t crs_mmap_mem_resize(crs_mmap_mem *mem,size_t extra_bytes) {
    ...
    if (mem->mapped == ANON_MAPPED) {
        #ifdef __linux__
            mem->data = mremap(mem->data, mem->len, new_size, MREMAP_MAYMOVE);
        #else
        unsigned char *new_data = mmap(NULL,
                            new_size,
                            PROT_READ | PROT_WRITE,
                            MAP_ANON | MAP_SHARED,
                            -1, 0);       memcpy(new_data,mem->data,mem->len);
        ...
        if (mem->data == 0xffffffffffffffff) {
                    mem->data = mmap(NULL,
                            new_size,
                            PROT_READ | PROT_WRITE,
                            MAP_SHARED,
...
void crs_restruct_mem_bin(crs_mmap_mem *mem,unsigned char **bin_cursor_p,guint *new_sizes) {
    ...
    *bin_cursor_p = bin_kb_cursor;
    ...
    if (rest_bin) {
        ...
        memmove(p + new_bytes, p, rest_bin);
    }
    ...
    cursor += offset; 
    memmove(cursor,crs_rowptr_head(bin_kb_cursor),crs_rowptr_len(bin_kb_cursor) * sizeof(guint));
    header[ROWPTR_OFFSET]+=offset;

    cursor =(unsigned char *) crs_rowlist_head(bin_kb_cursor);
    offset = extra_colind * sizeof(fs_rid); 
    cursor += offset;
    memmove(cursor,crs_rowlist_head(bin_kb_cursor),crs_rowlist_len(bin_kb_cursor) * sizeof(fs_rid));
    header[ROWLIST_OFFSET]+=offset;
    elapsed_res += g_timer_elapsed(ts_res,NULL);
    elapsed_res_count += 1;
    g_timer_destroy(ts_res);
}
...
void crs_restruct_mem(crs_mmap_mem *mem,guint *new_sizes) {
    ...
    offset = (extra_colind + extra_rowlist) * sizeof(fs_rid);
    cursor += offset; 
    memmove(cursor,crs_rowptr_head(mem->data),crs_rowptr_len(mem->data) * sizeof(guint));
    header[ROWPTR_OFFSET]+=offset;

    cursor =(unsigned char *) crs_rowlist_head(mem->data);
    offset = extra_colind * sizeof(fs_rid); 
    cursor += offset;
    memmove(cursor,crs_rowlist_head(mem->data),crs_rowlist_len(mem->data) * sizeof(fs_rid));
    header[ROWLIST_OFFSET]+=offset;
}


void mymemmove(unsigned char *dst,unsigned char *src,size_t len) {
    unsigned char *dst_end = dst + (len - 1);
    unsigned char *src_end = src + (len - 1);
    while(len--) *dst_end-- = *src_end--;
}
...
void crs_bin_insert_vals(crs_mmap_mem *mem,unsigned char **kb_cursor_p,unsigned char *new_start,size_t new_len){
    ...
    if (shift_bytes>0) 
        memmove(kb_cursor+new_len,kb_cursor,shift_bytes);
    memcpy(kb_cursor,new_start,new_len);
    *kb_cursor_p = kb_cursor;
    elapsed_ins += g_timer_elapsed(ts,NULL);
    elapsed_ins_count += 1;
    g_timer_destroy(ts);
}
...
void crs_colind_insert_vals(unsigned char *mem, fs_rid *vals, guint start, guint len) {
    ...
    memmove(colind + start + len, colind + start, (crs_colind_len(mem) - start) * sizeof(fs_rid));
...
crs_matrix *crs_matrix_merge(crs_matrix *kb_matrix, crs_matrix *new_matrix) {
    elapsed_res = 0.0; elapsed_res_count = 0; elapsed_ins = 0.0; elapsed_ins_count = 0;
    ...
    for(guint row_new_i=0 ; row_new_i< crs_rowlist_len(new_matrix->index->data) ; row_new_i++) {
        if (!(row_new_i % 20))
            log_debug("merging rows %u/%u",row_new_i,crs_rowlist_len(new_matrix->index->data));    
    ...
    log_debug("End elapsed_res (calls %i) time = %.2lf",elapsed_res_count,elapsed_res);
    log_debug("End elapsed_ins (calls %i) time = %.2lf",elapsed_ins_count,elapsed_ins);
...
crs_matrix *crs_load_matrix(gchar *file_crs,crs_matrix_index *index,guint segment,GError **error) {
    ...
    crs_matrix *crs = crs_io_load_matrix(file_crs,index,segment,&error_load_crs);
    if (error_load_crs) {
...
void crs_matrix_inverse_list_build(crs_matrix *m) {
    ...
    crs_matrix_inverse_list *il_row = crs_matrix_inverse_list_new();
    crs_matrix_inverse_list *il_col = crs_matrix_inverse_list_new();
    ...
    while(bin_it->i < (int) m_colind_len) {
        ...
        while (crs_matrix_iterator_next(mi)) {
           log_debug("xvxvxv(%i) %llx %llx",bin_it->i,mi->col,mi->row); 
           crs_matrix_inverse_list_append(il_row,mi->row,bin_it->i);
           crs_matrix_inverse_list_append(il_col,mi->col,bin_it->i);
        }
    }
    free(mi);
    ...
    crs_matrix_inverse_list_sort(il_col);
    crs_list_collapsed *col_collapsed = crs_list_collapsed_new(il_col);
    
    //crs_matrix_inverse_list_sort(il_row);
